...
# Load the stopwords from vietnamese-stopwords.txt
def load_stopwords(file_path):
    """Load stopwords from a file where each line contains one stop word."""
...
def generate_wordcloud(text_data, label_name, stopwords):
...
def plot_sentence_length_distribution(text_data):
    ...
    plt.title('Distribution of Sentence Lengths in Facebook Comment', fontsize=16)
    plt.xlabel('Sentence Length')
    plt.ylabel('Number of Sentences')
    plt.xlim(0, 150)  # Set x-axis limits from 0 to 150
...
def main():
    ...
    for label in unique_labels[:3]:  # Limit to first 3 labels
        ...
        generate_wordcloud(label_text_data, label, stopwords)
